...
/// Filter for selecting a specific BLE device during scan
#[derive(Debug, Clone)]
pub enum DeviceFilter {
    /// Match device by name (case-insensitive substring match)
    Name(String),
    /// Match device by BLE address (exact match, colon-separated hex)
    Address(String),
}

impl DeviceFilter {
    /// Parse a filter string — if it looks like a MAC address, use Address; otherwise Name
    pub fn parse(s: &str) -> Self {
        // MAC address pattern: XX:XX:XX:XX:XX:XX
        let parts: Vec<&str> = s.split(':').collect();
        if parts.len() == 6 && parts.iter().all(|p| p.len() == 2 && p.chars().all(|c| c.is_ascii_hexdigit())) {
            DeviceFilter::Address(s.to_uppercase())
        } else {
            DeviceFilter::Name(s.to_string())
        }
    }
    ...
    fn matches(&self, name: &str, address: &str) -> bool {
        match self {
            DeviceFilter::Name(filter) => name.to_lowercase().contains(&filter.to_lowercase()),
            DeviceFilter::Address(filter) => address.to_uppercase() == *filter,
        }
    }
}
...
impl BleLedDevice {
    ...
    #[instrument]
    pub async fn new(filter: Option<DeviceFilter>) -> Result<BleLedDevice> {
        let mut device = Self::new_without_power(filter).await?;
    ...
    #[instrument]
    pub async fn new_without_power(filter: Option<DeviceFilter>) -> Result<BleLedDevice> {
                    ...
                    if let Ok(Some(props)) = p.properties().await {
                        if let Some(name) = props.local_name {
                            ...
                            if device_type != DeviceType::Unknown {
                                // If filter provided, check if this device matches
                                if let Some(ref f) = filter {
                                    let addr = props.address.to_string();
                                    if f.matches(&name, &addr) {
                                        info!(
                                            "Found matching device: {} [{}] (type: {:?})",
                                            name, addr, device_type
                                        );
                                        ...
                                        debug!(
                                            "Skipping non-matching device: {} [{}]",
                                            name, addr
                                        );
                                    }
                                } else {
                                    // No filter — take first compatible device (original behavior)
                                    info!(
    ...
    /// Scan for all compatible BLE devices and return their info
    pub async fn scan_devices() -> Result<Vec<(String, String, DeviceType)>> {
        let manager = Manager::new().await?;
        let central = get_central(&manager).await?;
        ...
        for p in peripherals {
            if let Ok(Some(props)) = p.properties().await {
                if let Some(name) = props.local_name {
                    ...
                    if device_type != DeviceType::Unknown {
                        let addr = props.address.to_string();
                        devices.push((name, addr, device_type));
                    }
                }
            }
        }
