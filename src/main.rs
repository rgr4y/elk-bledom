...
struct Cli {
    /// Target a specific device by name (substring) or BLE address (XX:XX:XX:XX:XX:XX)
...
enum Commands {
    ...
    /// List all compatible BLE LED devices in range
    List,
...
async fn main() -> Result<()> {
    ...
    let device_filter = cli.device.map(|d| DeviceFilter::parse(&d));

    // Handle list command before device initialization (doesn't need a connection)
    if matches!(cli.command, Some(Commands::List)) {
        let devices = BleLedDevice::scan_devices().await?;
        if devices.is_empty() {
            println!("No compatible devices found.");
        } else {
            println!("{:<30} {:<20} {:<10}", "NAME", "ADDRESS", "TYPE");
            println!("{}", "-".repeat(60));
            for (name, addr, dtype) in &devices {
                ...
                println!("{:<30} {:<20} {:<10}", name, addr, type_name);
            }
        }
        return Ok(());
    }
    ...
    let mut device = match BleLedDevice::new_without_power(device_filter).await {
        Ok(dev) => dev,
        Err(e) => {
    ...
    match cli.command.unwrap_or(Commands::Demo { duration: 5 }) {
        ...
        Commands::Brightness { level } => {
            if level == 0 {
                device.power_off().await?;
                info!("Brightness 0: device powered off");
            } else {
                if !device.is_on {
                    device.power_on().await?;
                }
        ...
        Commands::Color { red, green, blue } => {
            if red == 0 && green == 0 && blue == 0 {
                device.power_off().await?;
                info!("Color RGB(0,0,0): device powered off");
            } else {
                if !device.is_on {
                    device.power_on().await?;
                }
        ...
        Commands::List => {
            // Already handled above; early return prevents reaching here
        }
        Commands::Audio {
            mode,
            range,
